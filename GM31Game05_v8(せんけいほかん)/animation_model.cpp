...
void CAnimationModel::Draw()
{
	...
	for (unsigned int m = 0; m < m_Scene->mNumMeshes; m++)
	{
		...
		aiMaterial* material = m_Scene->mMaterials[mesh->mMaterialIndex];					//添え字のためにmaterial呼び出し

		//テクスチャ設定
		aiString path;
		material->GetTexture(aiTextureType_DIFFUSE, 0, &path);								//これもm_Textureの添え字のため
		CRenderer::GetDeviceContext()->PSSetShaderResources(0, 1, &m_Texture[path.data]);	//添え字は文字型
...
void CAnimationModel::Load(const char *FileName)
{
	...
	m_Scene = aiImportFile(FileName,
		aiProcessPreset_TargetRealtime_MaxQuality | aiProcess_ConvertToLeftHanded);	//テクスチャのずれをaiProcess_ConvertToLeftHandedで修正
	...
	for (unsigned int m = 0; m < m_Scene->mNumMeshes; m++)
	{
		...
		for (unsigned int b = 0; b < mesh->mNumBones; b++)
		{
			...
			for (unsigned int w = 0; w < bone->mNumWeights; w++)
			{
				...
				assert(m_DeformVertex[m][weight.mWeight].BoneNum <= 4);
			}
		}
			...
			if (m_Scene->mMaterials[m]->GetTexture(aiTextureType_DIFFUSE, 0, &path) == AI_SUCCESS)
			{
				if (path.data[0] == '*')
				{
					...
					if (m_Texture[path.data] == NULL)
					{
						...
						int id = atoi(&path.data[1]); // a to i = ASCII to Int
...
void CAnimationModel::Update(const char *AnimationName1,const char *AnimationName2,float BlendRate,int Frame)
{
	...
	for (unsigned int m = 0; m < m_Scene->mNumMeshes; m++)
	{
		...
		for (unsigned int v = 0; v < mesh->mNumVertices; v++)
		{
			...
			//outMatrix = matrix[1] * deformVertex->BoneWeight[1]
			//outMatrix = matrix[2] * deformVertex->BoneWeight[2]
			//outMatrix = matrix[3] * deformVertex->BoneWeight[3]
